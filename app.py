...
IMPORTANT: Include ALL retrieved cases in your case_citations array with their exact details."""
        ...
        try:
            ...
            # Ensure citations are populated from RAG if empty
            if not argument.case_citations and relevant_docs:
                print(f"⚠️ {self.role.upper()}: No citations found - adding {len(relevant_docs[:3])} from RAG")
                argument.case_citations = [
                    CaseCitation(
                        ...
                        relevance=f"Retrieved case supporting {self.role} argument",
...
- confidence_score must be a number between 0.6 and 0.95"""
    ...
    try:
        ...
        # If no valid citations, add from RAG
        if not data.get('case_citations') and relevant_docs:
            data['case_citations'] = [
                {
                    'case_name': doc.metadata.get('case_name', 'Unknown Case'),
                    'citation': doc.metadata.get('citation', 'N/A'),
                    'year': doc.metadata.get('date', '2020')[:4] if doc.metadata.get('date') else '2020',
                    'relevance': f"Supports {role} position",
                    'excerpt': doc.page_content[:200] + "..."
                }
